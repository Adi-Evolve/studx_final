...
// Placeholder data
const roomTypes = ['Single Room', 'Double Room', 'Studio', '1 BHK', '2 BHK', '3 BHK', 'Shared Apartment'];
...
export default function RoomsForm({ initialData = {}, onSubmit, category = 'Rooms/Hostel', isEditMode = false }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Add category
            formDataToSend.append('category', formData.category || category || 'Rooms/Hostel');
            // Add flat-specific fields
            if (formData.category === 'Flat' || category === 'Flat') {
                if (formData.floor_number) formDataToSend.append('floor_number', formData.floor_number);
                if (formData.total_floors) formDataToSend.append('total_floors', formData.total_floors);
                if (formData.furnished_status) formDataToSend.append('furnished_status', formData.furnished_status);
                if (formData.area_sqft) formDataToSend.append('area_sqft', formData.area_sqft);
                formDataToSend.append('has_balcony', formData.has_balcony);
                formDataToSend.append('has_parking', formData.has_parking);
            }
            // Add amenities
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {/* Flat-Specific Fields */}
            {(formData.category === 'Flat' || category === 'Flat') && (
                <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-lg p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label htmlFor="floor_number" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Floor Number</label>
                            <input
                                type="number"
                                name="floor_number"
                                id="floor_number"
                        ...
                        <div>
                            <label htmlFor="total_floors" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Total Floors in Building</label>
                            <input
                                type="number"
                                name="total_floors"
                                id="total_floors"
                        ...
                        <div>
                            <label htmlFor="furnished_status" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Furnished Status</label>
                            <select
                        ...
                        <div>
                            <label htmlFor="area_sqft" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Area / Size</label>
                            <input
                                type="text"
                                name="area_sqft"
                                id="area_sqft"
                        ...
                        <div className="flex items-center gap-6">
                            <div className="flex items-center">
                                <input
                                    ...
                                    onChange={(e) => setFormData(prev => ({ ...prev, has_balcony: e.target.checked }))}
                                    className="h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500"
                                />
                                <label htmlFor="has_balcony" className="ml-2 block text-sm text-gray-900 dark:text-gray-100">Balcony</label>
                            </div>
                            <div className="flex items-center">
                                <input
                                    type="checkbox"
                                    ...
                                    className="h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500"
                                />
                                <label htmlFor="has_parking" className="ml-2 block text-sm text-gray-900 dark:text-gray-100">Parking Included</label>
                            </div>
                        </div>
                    </div>
                </div>
