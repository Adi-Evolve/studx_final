...
export default function GoogleMapPicker({ onLocationChange, initialPosition, initialLocation }) {
    ...
    const handleGetCurrentLocation = async () => {
        ...
        try {
            ...
            // Enhanced geolocation options for maximum precision
            const options = {
            ...
            // Show toast to indicate we're getting precise location
            const loadingToast = toast.loading('Getting your precise location using GPS...');
            ...
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    if (map) {
                        ...
                        map.setZoom(18);  // Increased zoom for precise location
                    }
                    ...
                    if (accuracy <= 20) {
                        toast.success(`📍 Precise location detected! (Accuracy: ${accuracy.toFixed(1)}m)`, { duration: 4000 });
                    } else if (accuracy <= 50) {
                        toast.success(`📍 Location detected with ${accuracy.toFixed(1)}m accuracy`, { duration: 4000 });
                    } else {
                        toast.success(`📍 Location detected (Accuracy: ${accuracy.toFixed(1)}m). Consider moving to an open area for better precision.`, { duration: 5000 });
                    }
                },
                (error) => {
                    setIsGettingLocation(false);
                    ...
                    switch(error.code) {
                        case error.PERMISSION_DENIED:
                            ...
                            errorSolution = 'Please enable location permissions in your browser settings and try again.';
                            break;
                        case error.POSITION_UNAVAILABLE:
                            errorMessage = 'Location information unavailable';
                            errorSolution = 'Please check your device location settings and ensure GPS is enabled.';
                            break;
                        case error.TIMEOUT:
                            errorMessage = 'Location request timed out';
                            errorSolution = 'Please try again. Move to an open area for better GPS signal.';
                            break;
                        default:
                    ...
                    toast.error(`${errorMessage}. ${errorSolution}`, { duration: 6000 });
                },
                options
            );
        } catch (error) {
