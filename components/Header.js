...
export default function Header() {
    ...
    // HIDDEN: Mess owner state - commented out but not deleted
    ...
    useEffect(() => {
        ...
        const getInitialSession = async () => {
            ...
            // HIDDEN: Check mess owner status - commented out but not deleted
            // if (session?.user) {
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
            ...
            if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') {
                router.refresh();
                // HIDDEN: Check mess owner status - commented out
                // if (session?.user) {
                //     checkMessOwnerStatus(session.user.id);
                // }
            } // HIDDEN: else if for signed out - commented out
    ...
    // HIDDEN: Check if user is a mess owner - commented out but not deleted
                                ...
                                <div className="relative" ref={profileMenuRef}>
                                    ...
                                    {isProfileMenuOpen && (
                                        <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
                                            ...
                                            {/* HIDDEN: Mess section - commented out but not deleted */}
                                            {/* {isMessOwner ? (
                                                <Link 
                                                    href="/profile/mess" 
                                                    className="block px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
