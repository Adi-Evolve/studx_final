...
class SponsorshipManager {
    ...
    /**
     * Mix sponsored items with regular items for search results
     * @param {Array} regularItems - Regular search results
     * @param {string} searchQuery - Search query
     * @param {Object} options - Options
     * @returns {Array} Mixed items array
     */
    async mixSponsoredWithRegular(regularItems, searchQuery, options = {}) {
        ...
        for (let i = 0; i < regularItems.length; i++) {
            // Add regular item
            mixedItems.push(regularItems[i]);

            // Insert sponsored item every N items
            if ((i + 1) % insertEvery === 0 && sponsoredIndex < sponsoredItems.length) {
                // Check if this sponsored item is not already in regular results
                const sponsoredItem = sponsoredItems[sponsoredIndex];
                const isDuplicate = regularItems.some(regular => 
                    regular.id === sponsoredItem.id && regular.type === sponsoredItem.type
                );
