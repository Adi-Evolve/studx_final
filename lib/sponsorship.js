...
class SponsorshipManager {
    ...
    /**
     * Mix sponsored items with regular items with GUARANTEED sponsored priority for search
     * @param {Array} regularItems - Regular search results
     * @param {string} searchQuery - Search query
     * @param {Object} options - Options
     * @returns {Array} Mixed items array with sponsored items ALWAYS first for search
     */
    async mixSponsoredWithRegular(regularItems, searchQuery, options = {}) {
        const { type = null, insertEvery = 5, maxSponsored = 5 } = options;

        // Get sponsored items that match the search query
        const sponsoredItems = await this.getSponsoredItems({
            searchQuery,
            type,
            limit: maxSponsored,
            excludeUsed: false // Allow sponsored items for search priority
        });

        if (sponsoredItems.length === 0) {
            // console.log('[sponsorshipManager] No sponsored items found, returning regular results');
            return regularItems;
        }

        // console.log(`[sponsorshipManager] Found ${sponsoredItems.length} sponsored items for query: "${searchQuery}"`);
        ...
        // Enhanced sponsored item filtering for search relevance
        let relevantSponsoredItems = sponsoredItems;
        
        if (searchQuery && searchQuery.trim()) {
            const lowerQuery = searchQuery.toLowerCase();
            const searchWords = lowerQuery.split(' ').filter(word => word.length > 0);
            
            // Score sponsored items for search relevance
            relevantSponsoredItems = sponsoredItems.map(item => {
                ...
                searchWords.forEach(word => {
                    if (title.includes(word)) relevanceScore += 50;
                    if (category.includes(word)) relevanceScore += 40;
                    if (description.includes(word)) relevanceScore += 20;
                    if (courseSubject.includes(word)) relevanceScore += 30;
                });
                
                // Exact phrase matches get higher scores
                if (title.includes(lowerQuery)) relevanceScore += 75;
                if (category.includes(lowerQuery)) relevanceScore += 60;
                if (description.includes(lowerQuery)) relevanceScore += 30;
                
                return {
                    ...item,
                    search_relevance_score: relevanceScore
                };
            })
            .filter(item => item.search_relevance_score > 0) // Only relevant sponsored items
            .sort((a, b) => {
                // Sort sponsored items by relevance, then by sponsor slot
                if (b.search_relevance_score !== a.search_relevance_score) {
                    return b.search_relevance_score - a.search_relevance_score;
                }
                return (a.sponsor_slot || 999) - (b.sponsor_slot || 999);
            });
        }

        // For search queries: ALWAYS place ALL relevant sponsored items at the top
        if (searchQuery && searchQuery.trim() && relevantSponsoredItems.length > 0) {
            // console.log(`[sponsorshipManager] Placing ${relevantSponsoredItems.length} sponsored items at top for search`);
            
            const sponsoredWithIndicators = relevantSponsoredItems.map((item, index) => ({
                ...item,
                is_sponsored: true,
                sponsored_label: 'Sponsored',
                sponsored_rank: index + 1, // Track sponsored ranking
                search_priority: 1000 + (100 - index) // Ensure they sort to top
            }));
            
            // Add search priority to regular items (lower priority)
            const regularWithPriority = filteredRegularItems.map((item, index) => ({
                ...item,
                search_priority: item.relevance_score || (500 - index)
            }));
            
            // Combine and sort to ensure sponsored items are always first
            const combinedResults = [...sponsoredWithIndicators, ...regularWithPriority];
            
            return combinedResults.sort((a, b) => {
                // Primary sort: sponsored items first (by search_priority)
                if (b.search_priority !== a.search_priority) {
                    return b.search_priority - a.search_priority;
                }
                // Secondary sort: by relevance or creation date
                if (a.is_sponsored && b.is_sponsored) {
                    return (a.sponsored_rank || 0) - (b.sponsored_rank || 0);
                }
                return new Date(b.created_at) - new Date(a.created_at);
            });
        }

        // For browsing (no search query): Mix sponsored items throughout
        // console.log('[sponsorshipManager] Mixing sponsored items for browsing');
